...
public class SpscGrowableArrayQueue<E> extends BaseSpscLinkedArrayQueue<E>
{
        ...
        final Supplier<? extends E> s)
    {
        ...
        if (lookAheadStep > 0)
        {
            ...
            if (null == UnsafeRefArrayAccess.lvElement(buffer, CircularArrayOffsetCalculator.calcElementOffset(index + 1, mask)))
            { // buffer is not full
                writeToQueue(buffer, v == null ? s.get() : v, index, offset);
            }
            else
            {
                ...
                final E[] newBuffer = CircularArrayOffsetCalculator.allocate((int) (2 * (mask + 1) + 1));

                producerBuffer = newBuffer;
                producerMask = LinkedArrayQueueUtil.length(newBuffer) - 2;

                final long offsetInNew = CircularArrayOffsetCalculator.calcElementOffset(index, producerMask);
