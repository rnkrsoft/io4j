...
import com.rnkrsoft.io._3rd.org.jctools.queues.MessagePassingQueueUtil;
...
abstract class BaseSpscLinkedAtomicArrayQueue<E> extends BaseSpscLinkedAtomicArrayQueueProducerColdFields<E> implements MessagePassingQueue<E>, QueueProgressIndicators {
    ...
    protected final AtomicReferenceArray<E> lvNextArrayAndUnlink(AtomicReferenceArray<E> curr) {
        final int offset = LinkedAtomicArrayQueueUtil.nextArrayOffset(curr);
        final AtomicReferenceArray<E> nextBuffer = (AtomicReferenceArray<E>) LinkedAtomicArrayQueueUtil.lvElement(curr, offset);
        // prevent GC nepotism
    ...
    public int fill(Supplier<E> s, int limit) {
        for (int i = 0; i < limit; i++) {
            ...
            final int offset = LinkedAtomicArrayQueueUtil.calcElementOffset(index, mask);
            // expected hot path
    ...
    public E poll() {
        ...
        final int offset = LinkedAtomicArrayQueueUtil.calcElementOffset(index, mask);
        // LoadLoad
        final Object e = LinkedAtomicArrayQueueUtil.lvElement(buffer, offset);
    ...
    private E newBufferPeek(final AtomicReferenceArray<E> buffer, final long index) {
        ...
        final long mask = LinkedAtomicArrayQueueUtil.length(nextBuffer) - 2;
        consumerMask = mask;
        final int offset = LinkedAtomicArrayQueueUtil.calcElementOffset(index, mask);
        // LoadLoad
