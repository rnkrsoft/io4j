...
import static com.rnkrsoft.io.buffer.util.internal.StringUtil.NEWLINE;
import static com.rnkrsoft.io.buffer.util.internal.StringUtil.isSurrogate;

/**
 * A collection of utility methods that is related with handling {@link ByteBuffer},
 * such as the generation of hex dump and swapping an integer's byte order.
 */
public final class ByteBufferUtil {

    private static final InternalLogger logger = InternalLoggerFactory.getInstance(ByteBufferUtil.class);
    ...
    /**
     * Compares the two specified buffers as described in {@link ByteBuffer#compareTo(ByteBuffer)}.
     * This method is useful when implementing a new buffer type.
     */
    ...
    private static long compareUintBigEndian(
            ByteBuffer bufferA, ByteBuffer bufferB, int aIndex, int bIndex, int uintCountIncrement) {
    ...
    /**
     * The default implementation of {@link ByteBuffer#indexOf(int, int, byte)}.
     * This method is useful when implementing a new buffer type.
     */
    ...
    /**
     * Read the given amount of bytes into a new {@link ByteBuffer} that is allocated from the {@link ByteBufferAllocator}.
     */
    public static ByteBuffer readBytes(ByteBufferAllocator alloc, ByteBuffer buffer, int length) {
        boolean release = true;
        ByteBuffer dst = alloc.buffer(length);
        try {
     ...
     * it to a {@link ByteBuffer} allocated with {@code alloc}.
     * @param alloc The allocator used to allocate a new {@link ByteBuffer}.
     * @param seq The characters to write into a buffer.
     * @return The {@link ByteBuffer} which contains the <a href="http://en.wikipedia.org/wiki/UTF-8">UTF-8</a> encoded
     * result.
     */
    public static ByteBuffer writeUtf8(ByteBufferAllocator alloc, CharSequence seq) {
        // UTF-8 uses max. 3 bytes per char, so calculate the worst case.
        ByteBuffer buf = alloc.buffer(seq.length() * MAX_BYTES_PER_CHAR_UTF8);
        writeUtf8(buf, seq);
        return buf;
    }
    ...
    /**
     * Encode a {@link CharSequence} in <a href="http://en.wikipedia.org/wiki/UTF-8">UTF-8</a> and write
     * it to a {@link ByteBuffer}.
     *
     * This method returns the actual number of bytes written.
     */
    public static int writeUtf8(ByteBuffer buf, CharSequence seq) {
        final int len = seq.length();
        buf.ensureWritable(len * MAX_BYTES_PER_CHAR_UTF8);

        for (;;) {
            if (buf instanceof AbstractByteBuffer) {
                return writeUtf8((AbstractByteBuffer) buf, seq, len);
            } else if (buf instanceof WrappedByteBuffer) {
                // Unwrap as the wrapped buffer may be an AbstractByteBuffer and so we can use fast-path.
                buf = buf.unwrap();
            } else {
    ...
    private static int writeUtf8(AbstractByteBuffer buffer, CharSequence seq, int len) {
     ...
     * @param alloc The allocator used to allocate a new {@link ByteBuffer}.
     * @param seq The characters to write into a buffer.
     * @return The {@link ByteBuffer} which contains the <a href="http://en.wikipedia.org/wiki/ASCII">ASCII</a> encoded
     * result.
     */
    public static ByteBuffer writeAscii(ByteBufferAllocator alloc, CharSequence seq) {
        // ASCII uses 1 byte per char
        ByteBuffer buf = alloc.buffer(seq.length());
        writeAscii(buf, seq);
        return buf;
    }
     ...
     * This method returns the actual number of bytes written.
     */
    public static int writeAscii(ByteBuffer buf, CharSequence seq) {
        // ASCII uses 1 byte per char
        final int len = seq.length();
        buf.ensureWritable(len);
        for (;;) {
            if (buf instanceof AbstractByteBuffer) {
                writeAscii((AbstractByteBuffer) buf, seq, len);
                break;
            } else if (buf instanceof WrappedByteBuffer) {
                // Unwrap as the wrapped buffer may be an AbstractByteBuffer and so we can use fast-path.
                buf = buf.unwrap();
    ...
    // Fast-Path implementation
    private static void writeAscii(AbstractByteBuffer buffer, CharSequence seq, int len) {
    ...
    /**
     * Encode the given {@link CharBuffer} using the given {@link Charset} into a new {@link ByteBuffer} which
     * is allocated via the {@link ByteBufferAllocator}.
     */
    public static ByteBuffer encodeString(ByteBufferAllocator alloc, CharBuffer src, Charset charset) {
        return encodeString0(alloc, false, src, charset);
    }

    static ByteBuffer encodeString0(ByteBufferAllocator alloc, boolean enforceHeap, CharBuffer src, Charset charset) {
        ...
        try {
            final java.nio.ByteBuffer dstBuf = dst.internalNioBuffer(dst.readerIndex(), length);
    ...
    static String decodeString(ByteBuffer src, int readerIndex, int len, Charset charset) {
        if (len == 0) {
        ...
        if (src.nioBufferCount() == 1) {
            ...
            ByteBuffer buffer = src.alloc().heapBuffer(len);
            try {
    ...
    private static void decodeString(CharsetDecoder decoder, java.nio.ByteBuffer src, CharBuffer dst) {
        try {
     ...
     */
    public static ByteBuffer threadLocalDirectBuffer() {
    ...
    static final class ThreadLocalUnsafeDirectByteBuf extends UnpooledUnsafeDirectByteBuffer {
        ...
        private ThreadLocalUnsafeDirectByteBuf(Handle handle) {
            super(UnpooledByteBufferAllocator.DEFAULT, 256, Integer.MAX_VALUE);
            this.handle = handle;
        }
    ...
    static final class ThreadLocalDirectByteBuf extends UnpooledDirectByteBuffer {
    ...
    private static class IndexOfProcessor implements ByteBufferProcessor {
    ...
    /**
     * Returns {@code true} if the given {@link ByteBuffer} is valid text using the given {@link Charset},
     * otherwise return {@code false}.
     *
     * @param buf The given {@link ByteBuffer}.
    ...
    /**
     * Returns {@code true} if the specified {@link ByteBuffer} starting at {@code index} with {@code length} is valid
        ...
        if (charset.equals(CharsetUtil.UTF_8)) {
            ...
            try {
                if (buf.nioBufferCount() == 1) {
                    ...
                    ByteBuffer heapBuffer = buf.alloc().heapBuffer(length);
                    try {
     ...
     */
    private static final ByteBufferProcessor FIND_NON_ASCII = new ByteBufferProcessor() {
        @Override
        public boolean process(byte value) {
            return value >= 0;
        }
     ...
     * @param length The length of the specified buffer.
     */
    private static boolean isAscii(ByteBuffer buf, int index, int length) {
     ...
     * </pre>
     */
    private static boolean isUtf8(ByteBuffer buf, int index, int length) {
