...
import static com.rnkrsoft.io.buffer.DiskSizeUnit.*;
...
public abstract class ByteBuffers {
     ...
     * 使用缓存数组的最大字节大小
     */
    public static final int MAX_BUFFER_LENGTH = nKB(1);
    /**
     * 倍增扩容的阻断阈值，默认为4MB
     */
    public static final int DOUBLE_EXPAND_THRESHOLD = nMB(4);
    /**
     * 默认进行按需分配的其实阈值，默认8MB
     */
    public static final int DISTRIBUTION_ON_DEMAND_EXPAND_THRESHOLD = nMB(8);
    /**
    ...
    public static boolean DEFAULT_POOLED = false;
    ...
    public static final ByteBuffer newBuffer(ByteBufferType type, boolean pooled, int initCapacity, int maxCapacity) {
        if (type == ByteBufferType.DIRECT) {
            return pooled ? new PooledDirectByteBuffer(initCapacity, maxCapacity) : new UnpooledDirectByteBuffer(initCapacity, maxCapacity);
        } else if (type == ByteBufferType.HEAP) {
            return pooled ? new PooledHeapByteBuffer(initCapacity, maxCapacity) : new UnpooledHeapByteBuffer(initCapacity, maxCapacity);
        } else {
    ...
    /**
     * 将一个数组包装成缓冲区，可选择包装成基于堆的或者基于直接内存的
     *
     * @param type  缓存类型
     * @param bytes 待包装数组
     * @return 缓冲区对象
     */
    public static final ByteBuffer wrap(ByteBufferType type, byte[] bytes) {
        if (type == ByteBufferType.DIRECT) {
            return new UnpooledDirectByteBuffer(bytes, 0, bytes.length);
        } else if (type == ByteBufferType.HEAP) {
            return new UnpooledHeapByteBuffer(bytes, 0, bytes.length);
        } else {
            throw new IllegalArgumentException("illegal argument type:" + type);
        }
    }
    ...
    public static final ByteBuffer wrap(byte[] bytes) {
        return wrap(ByteBufferType.HEAP, bytes);
    }
