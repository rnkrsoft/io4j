 ...
 * recommended to use {@link ByteBuffer#slice()} and
...
class SlicedByteBuffer extends AbstractDerivedByteBuffer {
    ...
    public SlicedByteBuffer(ByteBuffer buffer, int index, int length) {
        super(length);
        if (index < 0 || index > buffer.capacity() - length) {
            throw new IndexOutOfBoundsException(buffer + ".slice(" + index + ", " + length + ')');
        }

        if (buffer instanceof SlicedByteBuffer) {
            this.buffer = ((SlicedByteBuffer) buffer).buffer;
            adjustment = ((SlicedByteBuffer) buffer).adjustment + index;
        } else if (buffer instanceof DuplicatedByteBuffer) {
