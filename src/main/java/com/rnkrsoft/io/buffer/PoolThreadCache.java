...
import com.rnkrsoft.io.buffer.PoolArena.SizeClass;
import com.rnkrsoft.io.buffer.util.Recycler;
import com.rnkrsoft.io.buffer.util.Recycler.Handle;
import com.rnkrsoft.io.buffer.util.internal.MathUtil;
...
final class PoolThreadCache {
     ...
     */
    boolean allocateTiny(PoolArena<?> area, PooledByteBuffer<?> buf, int reqCapacity, int normCapacity) {
     ...
     */
    boolean allocateSmall(PoolArena<?> area, PooledByteBuffer<?> buf, int reqCapacity, int normCapacity) {
     ...
     * Try to allocate a small buffer out of the cache. Returns {@code true} if successful {@code false} otherwise
     */
    boolean allocateNormal(PoolArena<?> area, PooledByteBuffer<?> buf, int reqCapacity, int normCapacity) {
        return allocate(cacheForNormal(area, normCapacity), buf, reqCapacity);
    }

    @SuppressWarnings({ "unchecked", "rawtypes" })
    private boolean allocate(MemoryRegionCache<?> cache, PooledByteBuffer buf, int reqCapacity) {
        if (cache == null) {
    ...
    private abstract static class MemoryRegionCache<T> {
         ...
         */
        public final boolean allocate(PooledByteBuffer<T> buf, int reqCapacity) {
