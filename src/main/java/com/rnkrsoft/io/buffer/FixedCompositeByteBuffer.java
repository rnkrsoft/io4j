...
import com.rnkrsoft.io.buffer.util.internal.RecyclableArrayList;
...
/**
 * {@link ByteBuffer} implementation which allows to wrap an array of {@link ByteBuffer} in a read-only mode.
 * This is useful to write an array of {@link ByteBuffer}s.
 */
final class FixedCompositeByteBuffer extends AbstractReferenceCountedByteBuffer {
    private static final ByteBuffer[] EMPTY = { Unpooled.EMPTY_BUFFER };
    ...
    FixedCompositeByteBuffer(ByteBufferAllocator allocator, ByteBuffer... buffers) {
    ...
    /**
     * Return the {@link ByteBuffer} stored at the given index of the array.
     */
    ...
    public ByteBuffer copy(int index, int length) {
        ...
        ByteBuffer buf = alloc().buffer(length);
        try {
    ...
    public java.nio.ByteBuffer[] nioBuffers(int index, int length) {
        ...
        try {
            ...
            return array.toArray(new java.nio.ByteBuffer[array.size()]);
        } finally {
            array.recycle();
        }
    }
