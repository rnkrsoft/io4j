...
final class PoolChunk<T> implements PoolChunkMetric {
    ...
    void initBuf(PooledByteBuffer<T> buf, long handle, int reqCapacity) {
        int memoryMapIdx = memoryMapIdx(handle);
        int bitmapIdx = bitmapIdx(handle);
        if (bitmapIdx == 0) {
    ...
    void initBufWithSubpage(PooledByteBuffer<T> buf, long handle, int reqCapacity) {
        initBufWithSubpage(buf, handle, bitmapIdx(handle), reqCapacity);
    }

    private void initBufWithSubpage(PooledByteBuffer<T> buf, long handle, int bitmapIdx, int reqCapacity) {
        assert bitmapIdx != 0;
