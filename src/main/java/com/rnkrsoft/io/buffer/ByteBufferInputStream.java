 ...
 * updating {@link ByteBuffer#writerIndex()} will not affect the return
...
public class ByteBufferInputStream extends InputStream implements DataInput {
     ...
     * {@code writerIndex}.
     * @param buffer The buffer which provides the content for this {@link InputStream}.
     * @param releaseOnClose {@code true} means that when {@link #close()} is called then {@link ByteBuffer#release()} will
     ...
     *            {@code writerIndex}
     */
    public ByteBufferInputStream(ByteBuffer buffer, int length, boolean releaseOnClose) {
        if (buffer == null) {
